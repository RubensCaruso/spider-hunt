...
// Função para salvar a página com o conteúdo das notas
function saveFullPage() {
    const userConfirmed = confirm("Você deseja salvar o conteúdo da mesa e das notas?");

    if (userConfirmed) {
        const tableArea = document.querySelector('#table-area');

        const savedContent = tableArea.innerHTML;
        localStorage.setItem('savedContent', savedContent);  // Salva o HTML da mesa no localStorage

        // Salvar o conteúdo de cada nota, incluindo posição
        const notes = document.querySelectorAll('.note');
        const notesData = Array.from(notes).map(note => ({
            content: note.innerText,  // Conteúdo da nota
            left: note.style.left || "0px", // Posição horizontal (se não definido, usa 0px)
            top: note.style.top || "0px"    // Posição vertical (se não definido, usa 0px)
        }));
...
function loadFullPage() {
    ...
    const savedNotesData = localStorage.getItem('notesData');  // Recupera os dados das notas
    ...
    const contentArea = document.querySelector('#table-area');
    contentArea.innerHTML = savedContent;  // Carrega o conteúdo salvo na área específica

    // Remove os dados da mesa ao carregar
    const diceElements = contentArea.querySelectorAll('.dice-element');
    diceElements.forEach(die => die.remove());

    // Remove notas existentes antes de restaurar as novas
    const existingNotes = contentArea.querySelectorAll('.note');
    existingNotes.forEach(note => note.remove());

    // Restaurar o conteúdo das notas
    if (savedNotesData) {
        const notesData = JSON.parse(savedNotesData);
        notesData.forEach(noteData => {
            const noteContainer = document.createElement('div');
            noteContainer.classList.add('note');
            noteContainer.contentEditable = "true"; // Permite edição de texto
            noteContainer.innerText = noteData.content;  // Restaurar conteúdo da nota
            
            // Restaurar a posição horizontal e vertical
            noteContainer.style.position = 'absolute';
            noteContainer.style.left = noteData.left;  // A posição horizontal deve ser um valor em pixels (ex: "100px")
            noteContainer.style.top = noteData.top;    // A posição vertical deve ser um valor em pixels (ex: "100px")
            
            // Adicionar funcionalidade de arrastar à nota restaurada
            addDragFunctionality(noteContainer);
            contentArea.appendChild(noteContainer);  // Adiciona a nota restaurada à mesa
        });
    }

    // Selecionar todos os itens restaurados na ordem correta
    const restoredItems = contentArea.querySelectorAll('.card, .note'); // Inclui cartas e notas

    // Converter NodeList para Array e ordenar por z-index
    const sortedItems = Array.from(restoredItems).sort((a, b) => {
        ...
        return zIndexA - zIndexB; // Ordena da camada mais baixa para a mais alta
    });

    // Adiciona evento de clique a cada item restaurado
    sortedItems.forEach(item => {
        item.addEventListener('click', (e) => selectCardOnTable(item));
    });

    // Simula o clique em cada item na ordem correta de camada
    sortedItems.forEach(item => {
        item.click(); // Simula o clique após adicionar o evento
    });

    reassignCardEvents();  // Reatribui eventos para cartas
    reassignTableEvents(); // Reatribui eventos da mesa (se necessário)
    reassignNoteEvents();  // NOVO: Reatribui eventos para notas
}
...
window.onload = function() {
    ...
    // Auto Save - executa a cada 10 minuto (600000 milissegundos)
...
// Função para reatribuir os eventos para notas após o conteúdo ser carregado
function reassignNoteEvents() {
    const notes = document.querySelectorAll('.note');
    notes.forEach(note => {
        // Reatribui o arrasto para a nota
        addDragFunctionality(note);

        // Reatribui o evento de foco ao clicar (para edição de texto)
        note.addEventListener('click', (event) => {
            event.stopPropagation();
            if (isEditingMode) {
                note.focus();  // Coloca o foco na nota para edição
            }
        });
    });
}
...
function reassignCardEvents() {
    // A função que reatribui os eventos para cartas após o conteúdo ser carregado
    const cards = document.querySelectorAll('.card');
    cards.forEach(card => {
        card.addEventListener('click', function() {
            console.log("Carta clicada!");
            // Adicione o comportamento que você deseja
        });
    });
}

function reassignTableEvents() {
    // A função que reatribui os eventos para a mesa após o conteúdo ser carregado
    const tableElements = document.querySelectorAll('.table-element');
    tableElements.forEach(element => {
        element.addEventListener('drag', function() {
            console.log("Elemento da mesa arrastado!");
            // Adicione o comportamento de arrastar que deseja
        });
    });
}
...
function reassignCardEvents() {
    const cards = document.querySelectorAll('.card');  // Seleciona todas as cartas na mesa

    cards.forEach(card => {
        // Evento de clique simples para selecionar a carta
        card.addEventListener('click', (e) => selectCardOnTable(card));

        // Evento de duplo clique (para dispositivos desktop)
        card.addEventListener('dblclick', () => openCardOptions(card));

        // Evento de toque para dispositivos móveis (detecta toque duplo)
        card.addEventListener('touchstart', handleTouch);

        // Eventos de clique longo
        card.addEventListener('mousedown', () => handleLongPress(card));
        card.addEventListener('touchstart', () => handleLongPress(card));

        // Cancelar o clique longo quando o usuário solta (mouseup/touchend)
        card.addEventListener('mouseup', clearLongPressTimeout);
        card.addEventListener('mouseleave', clearLongPressTimeout);
        card.addEventListener('touchend', clearLongPressTimeout);
        card.addEventListener('touchcancel', clearLongPressTimeout);

        // Cancela o toque longo se houver movimento
        card.addEventListener('mousemove', clearLongPressTimeout);
        card.addEventListener('touchmove', clearLongPressTimeout);

        // Adicionar eventos para arrastar a carta
...
function reassignTableEvents() {
    ...
    // Evento para arrastar a mesa com o mouse
    ...
    // Arrastar com toque (dispositivos móveis)
...
function deselectCurrentDie() {
    ...
    if (i === 0 && conjuntoNovoDados === 0) {
        ...
        cardContainer.style.left = `${190}px`;
        cardContainer.style.top = `${1000}px`;
     ...
     cardContainer.style.top = `${300}px`;
       ...
       //  Posição padrão se o deck não estiver mapeado
        cardContainer.style.left = `${55}px`;
        cardContainer.style.top = `${420}px`;
    }
...
//conjunto 4 de dados....outra cor
        ...
        cardContainer.style.left = `${10}px`;
            cardContainer.style.top = `${400}px`;
    } else if (i === 1 && conjuntoNovoDados === 3) {
        cardContainer.style.left = `${10}px`;
            cardContainer.style.top = `${1150}px`;
...
function duplicateCard() {
    ...
    const clone = selectedCard.cloneNode(true); // Clona a carta selecionada
    clone.style.left = `${parseInt(selectedCard.style.left) + 20}px`; // Ajusta a posição para não sobrepor
    clone.style.top = `${parseInt(selectedCard.style.top) + 20}px`;
    clone.setAttribute('data-locked', 'false'); // Destrava a nova carta clonada
    clone.classList.remove('locked'); // Remove a classe de bloqueio, se houver

    const tableArea = document.getElementById('table-area');
    tableArea.appendChild(clone); // Adiciona a nova carta à mesa

    // Eventos de clique para selecionar a carta e abrir o popup
    clone.addEventListener('click', (e) => selectCardOnTable(clone));
    clone.addEventListener('dblclick', () => openCardOptions(clone));

    // Configura os eventos de movimento (drag) e auto-scroll para a nova carta duplicada
    clone.addEventListener('mousedown', onMouseDown);
    clone.addEventListener('touchstart', onTouchStart);

    // Detecção de toque duplo para dispositivos móveis
    clone.addEventListener('touchstart', handleTouch);

    // Configura o evento de clique longo para abrir a janela de popup ampliada
    clone.addEventListener('mousedown', () => {
        handleLongPress(clone);  // Inicia o temporizador do clique longo
    });
    clone.addEventListener('touchstart', () => {
        handleLongPress(clone);  // Inicia o temporizador do toque longo
    });

    // Cancela o clique/toque longo quando o usuário solta
...
function deleteCard() {
    ...
    const confirmDelete = confirm("Tem certeza de que deseja apagar esta carta?");
    if (confirmDelete) {
        selectedCard.remove(); // Remove a carta da mesa
        selectedCard = null; // Limpa a seleção
    }

    closeCardOptions(); // Fecha o popup após apagar
}
...
// Variável para manter o estado atual do modo (escrever ou mover)
let isEditingMode = true;

// Função para criar uma nova nota de anotação
function createNote() {
    const tableArea = document.getElementById('table-area');
    const noteContainer = document.createElement('div');
    noteContainer.classList.add('note');
    ...
    // Estilo inicial para posicionar as notas
    noteContainer.style.position = 'absolute';
    noteContainer.style.left = `${10}px`;
    noteContainer.style.top = `${10}px`;

    // Tornar a nota arrastável e editável
    addDragFunctionality(noteContainer);

    tableArea.appendChild(noteContainer);
}

// Função para permitir o arrasto das notas (mouse e toque)
function addDragFunctionality(element) {
    let startX, startY, offsetX, offsetY;
    let isDragging = false;

    // Variáveis de AutoScroll
    let scrollDirection = null;
    let autoScrollInterval = null;

    // Adicionar foco para digitar (só se estiver no modo editar)
    element.addEventListener('click', (event) => {
        event.stopPropagation(); // Impede que o clique afete a mesa
        if (isEditingMode) {
            element.focus(); // Coloca o foco na nota para permitir digitação
        }
    });

    // Prevenir que o evento de mousedown nas notas mova a mesa (para mouse)
    element.addEventListener('mousedown', (event) => {
        event.preventDefault();  // Previne comportamento padrão
        event.stopPropagation(); // Impede que o evento afete a mesa
        ...
        startAutoScroll(); // Iniciar o AutoScroll
    });

    // Prevenir que o evento de touchstart nas notas mova a mesa (para toque)
    element.addEventListener('touchstart', (event) => {
        ...
        // Evitar conflito com digitação: verificar se o usuário está tocando para arrastar
        if (isEditingMode) {
            element.focus(); // Coloca o foco para digitação
        } else {
            document.addEventListener('touchmove', onTouchMove);
            document.addEventListener('touchend', onTouchEnd);
    ...
    // Movimentar a nota com base no zoom atual (para mouse)
    function onMouseMove(event) {
        if (!isDragging) return;
        const tableArea = document.getElementById('table-area');
        const tableRect = tableArea.getBoundingClientRect();
        let zoomLevel = parseFloat(tableArea.style.transform.replace('scale(', '').replace(')', '')) || 1;

        let newLeft = (event.clientX - tableRect.left - startX) / zoomLevel;
        let newTop = (event.clientY - tableRect.top - startY) / zoomLevel;

        const maxLeft = (tableRect.width - element.offsetWidth) / zoomLevel;
        const maxTop = (tableRect.height - element.offsetHeight) / zoomLevel;

        
        element.style.left = `${Math.min(Math.max(newLeft, 0), maxLeft)}px`;
        element.style.top = `${Math.min(Math.max(newTop, 0), maxTop)}px`;

        handleAutoScroll(event.clientX, event.clientY);  // Aciona o AutoScroll enquanto move
    }

    // Movimentar a nota com base no zoom atual (para toque)
    function onTouchMove(event) {
        if (!isDragging) return;
        const touch = event.touches[0];
        const tableArea = document.getElementById('table-area');
        ...
        let newLeft = (touch.clientX - tableRect.left - startX) / zoomLevel;
        let newTop = (touch.clientY - tableRect.top - startY) / zoomLevel;
        ...
        handleAutoScroll(touch.clientX, touch.clientY);  // Aciona o AutoScroll enquanto move
    }

    // Finalizar o arrasto (para mouse)
    function onMouseUp() {
        isDragging = false;
        document.removeEventListener('mousemove', onMouseMove);
        document.removeEventListener('mouseup', onMouseUp);
        stopAutoScroll(); // Parar o AutoScroll
    }

    // Finalizar o arrasto (para toque)
    function onTouchEnd() {
        isDragging = false;
        document.removeEventListener('touchmove', onTouchMove);
        document.removeEventListener('touchend', onTouchEnd);
    ...
    // Função para definir a direção do AutoScroll
    function handleAutoScroll(clientX, clientY) {
        const tableWrapper = document.getElementById('table-wrapper');
        const rect = tableWrapper.getBoundingClientRect();
...
// Adicionar evento para criar notas na mesa ao clicar em um botão
document.getElementById('create-note-btn').addEventListener('click', createNote);

// Adicionar evento para alternar entre modos de "escrever" e "mover"
document.getElementById('edit-mode-btn').addEventListener('click', () => {
    isEditingMode = !isEditingMode;  // Alterna entre editar e mover
    document.getElementById('edit-mode-btn').innerText = isEditingMode ? 'Modo Ativo: Escrever' : 'Modo Ativo: Mover'; // Muda o texto do botão
});
